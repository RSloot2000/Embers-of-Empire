...
foreach ($cf in $compFiles) {
    ...
    $srcs = @{"CFP"=$cfp; "EPE"=$epe; "CFP+EPE"=$cfpepe; "RENO"=$reno}
    foreach ($sn in $srcs.Keys) {
        $sp = Join-Path $srcs[$sn] $cf
        if (Test-Path $sp) {
            ...
            if ($miss.Count -gt 0) {
                $out += "  $sn : $($stTpl.Count) total, $($miss.Count) MISSING: $($miss -join ', ')"
            } else {
                $out += "  $sn : $($stTpl.Count) total, all in compatch"
            }
        }
    }
    $out += ""
...
$out | Out-File "c:\Users\ruben\Desktop\EoE_mod_dev\EoE+compatches\refs\portrait_template_check.txt" -Encoding utf8
Write-Host "Done. Output: refs/portrait_template_check.txt"
